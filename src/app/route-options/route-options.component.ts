  ...
  ngOnInit() {
    this.geoLocationService.getGeoLocation().then(position => {
      for (const routeOption of this.routeOptions) {
        this.distanceMatrixApiService.getTimeAndDistance({lat: position.coords.latitude, lng: position.coords.longitude}, ALMERE_FLORIADE, routeOption.travelMode).subscribe(response => {
          routeOption.travelTime = response.rows[0].elements[0].duration.text;
          routeOption.travelDistance = response.rows[0].elements[0].distance.text;
        });
      }
    }, error => {
      console.error(error.message);
      for (const routeOption of this.routeOptions) {
        this.distanceMatrixApiService.getTimeAndDistance(ALMERE_CENTRUM, ALMERE_FLORIADE, routeOption.travelMode).subscribe(response => {
          routeOption.travelTime = response.rows[0].elements[0].duration.text;
          routeOption.travelDistance = response.rows[0].elements[0].distance.text;
        });
      }
